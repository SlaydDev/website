  ...
  function hideCover() {
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        animateCounters();
        loadBlogPreview();
        loadCounters(); // fetch dynamic counters
      },500);
    },300);
  }
});
...
function revealOnScroll(){
  ...
  elems.forEach(el=>{const rect=el.getBoundingClientRect(); if(rect.top<window.innerHeight-60){el.classList.add("visible");}});
}
...
window.addEventListener('keydown',e=>{if(e.key==="Escape") blogOverlay.style.display='none';});

// FETCH COUNTERS
async function loadCounters(){
  try{
    const response=await fetch("https://script.google.com/macros/s/AKfycbwG3r12rcr5XF0O6VbVdkvWXY-lp64NztseAPcdo3c1YVa_v4IlEIjSSLWzfi7t342vag/exec");
    const data=await response.json();
    document.querySelector("#stats .stat:nth-child(2) .counter").dataset.target=data.ProjectsDone;
    document.querySelector("#stats .stat:nth-child(3) .counter").dataset.target=data.OngoingProjects;
    document.querySelector("#stats .stat:nth-child(4) .counter").dataset.target=data.HappyCustomers;
    animateCounters();
